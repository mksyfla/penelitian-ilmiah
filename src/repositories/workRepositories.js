...
async function getWorkByIdRepository({ workId, jobId }) {
  const query = {
    text: `
    SELECT w.id, w.title, w.content, w.is_choose, w.image, w.created_at as createdAt, u.name, j.id as job_id
    FROM works as w
  ...
  if (!result.rowCount) throw new NotFoundError('karya tidak ditemukan');
...
async function postWorkRepository({
  ...
  const query = {
    text: `
    INSERT INTO works(title, content, is_choose, image, created_at, updated_at, job_id, user_id)
  ...
  if (!result.rows[0].id) throw new InvariantError('karya gagal ditambahkan');
...
async function putWorkByIdRepository({
  ...
  const query = {
    ...
    values: [title, content, image, updatedAt, workId, userId],
  };
...
async function deleteWorkByIdRepository({ workId, userId }) {
  const query = {
  ...
  if (!result.rowCount) throw new InvariantError('kriteria gagal dihapus');
}

async function verifyOwnerWork({ userId }) {
  const query = {
...
async function verifyWorkExist({ workId }) {
  const query = {
...
async function getImagePath({ workId }) {
  const query = {
...
async function workChoose({ workId, jobId }) {
  const query = {
